...
import { InfinityQuery, InfinityColumn, GrafanaTableRow, InfinityCSVQuery } from './../../types';

export class CSVParser extends InfinityParser {
  constructor(CSVResponse: string, target: Extract<InfinityQuery, InfinityCSVQuery>, endTime?: Date) {
    super(target);
  ...
  private formatInput(CSVResponse: string) {
    if (this.target.type === 'csv') {
      const options = {
        columns:
          this.target.csv_options && this.target.csv_options.columns
            ? this.target.csv_options.columns.split(',')
            : true,
        delimiter:
