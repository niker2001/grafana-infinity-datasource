...
import { InfinityQuery, InfinityColumn, GrafanaTableRow, GrafanaTableRowItem, InfinityHTMLQuery } from './../../types';

export class HTMLParser extends InfinityParser {
  constructor(HTMLResponse: string, target: Extract<InfinityQuery, InfinityHTMLQuery>, endTime?: Date) {
    super(target);
  ...
  private formatInput(HTMLResponse: string) {
    ...
    const rootElements = this.target.type === 'html' ? $(this.target.root_selector) : '';
    return rootElements;
  }
  private constructTableData(rootElements: cheerio.Cheerio | '') {
    if (this.target.type === 'html' && typeof rootElements !== 'string') {
