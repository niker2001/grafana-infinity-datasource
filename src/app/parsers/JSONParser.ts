...
import { InfinityQuery, InfinityColumn, GrafanaTableRow, InfinityJSONQuery, InfinityGraphqlQuery } from './../../types';
import { getColumnsFromObjectArray, columnarToTable } from './utils';

export class JSONParser extends InfinityParser {
  constructor(
    JSONResponse: object,
    target: Extract<InfinityQuery, InfinityJSONQuery | InfinityGraphqlQuery>,
    endTime?: Date
  ) {
    super(target);
    let jsonResponse = this.formatInput(JSONResponse);
    if (this.target.type === 'json' || this.target.type === 'graphql') {
