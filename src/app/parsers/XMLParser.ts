...
import { InfinityQuery, InfinityColumn, GrafanaTableRow, InfinityXMLQuery } from './../../types';

export class XMLParser extends InfinityParser {
  constructor(XMLResponse: any | string, target: Extract<InfinityQuery, InfinityXMLQuery>, endTime?: Date) {
    super(target);
    this.formatInput(XMLResponse).then((xmlResponse: any) => {
      if (this.target.type === 'xml') {
